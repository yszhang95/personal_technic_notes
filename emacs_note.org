...
** Link
   ...
   =org-store-link= will store a link to the current location. For example,
   put the =point= under =(provide 'init-org)= in
   =~/.emacs.d/lisp/init-org.el=. Run this command, you will find the following
   message.
   #+begin_example
